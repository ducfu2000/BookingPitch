...
export default function HistoryBookings({navigation, route}) {
  ...
  const HistoryAwait = _currentPage => {
    setIsLoading(true);
    fetch(
      `${host}/api/tenant/booking/history?status=Confirmed&condition=future&page=${_currentPage}`,
      {
        method: 'GET',
        headers: {
  ...
  return (
    <SafeAreaView style={BaseStyles.root}>
      <ScrollView>
        ...
        {bookingHistory.length % 9 == 0 && !bookingHistory.length == 0 ? (
          <View style={[{alignItems: 'center'}]}>
            <Icon name="md-filter" size={18} />
            <Text
              onPress={() => {
